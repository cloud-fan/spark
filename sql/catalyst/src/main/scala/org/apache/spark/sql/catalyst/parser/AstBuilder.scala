...
/**
  ...
  private def withMatchRecognize(
    ...
    val allRowsPerMatch = Option(ctx.rowsPerMatchClause()) match {
      case Some(clause) => clause match {
        case _: OneRowPerMatchContext => false
        case _: AllRowsPerMatchShowEmptyContext => true
        case _: AllRowsPerMatchOmitEmptyContext =>
          throw QueryParsingErrors.unsupportedMatchRecognizeRowsPerMatchMode(
            "ALL ROWS PER MATCH OMIT EMPTY MATCHES", clause)
        case _: AllRowsPerMatchUnmatchedRowsContext =>
          throw QueryParsingErrors.unsupportedMatchRecognizeRowsPerMatchMode(
            "ALL ROWS PER MATCH WITH UNMATCHED ROWS", clause)
      }
      case None => false
    }

    // Validate AFTER MATCH SKIP clause: only SKIP PAST LAST ROW is supported
    Option(ctx.afterMatchSkipClause()).foreach {
      case _: SkipPastLastRowContext => // default, supported
      case c: SkipToNextRowContext =>
        throw QueryParsingErrors.unsupportedMatchRecognizeAfterMatchSkipMode(
          "AFTER MATCH SKIP TO NEXT ROW", c)
      case c: SkipToFirstVariableContext =>
        throw QueryParsingErrors.unsupportedMatchRecognizeAfterMatchSkipMode(
          "AFTER MATCH SKIP TO FIRST", c)
      case c: SkipToLastVariableContext =>
        throw QueryParsingErrors.unsupportedMatchRecognizeAfterMatchSkipMode(
    ...
    val result = UnresolvedMatchRecognize(
      ...
      measures,
      allRowsPerMatch,
      virtualColumns = MatchRecognizeVirtualColumns.empty, // Populated during analysis
      query)
  ...
  override def visitRowPatternFactor(ctx: RowPatternFactorContext): RowPattern = withOrigin(ctx) {
    ...
    } else if (ctx.PERMUTE != null) {
      val patterns = ctx.rowPattern.asScala.map(visitRowPattern).toSeq
      if (patterns.size < 2) {
        throw QueryParsingErrors.matchRecognizePermuteTooFewPatternsError(ctx)
      }
      PatternPermute(patterns)
    } else {
      // Grouped pattern: (pattern)
      visitRowPattern(ctx.rowPattern.get(0))
    }
  }
  ...
  private def visitRowPatternQuantifierToQuantifier(
    ...
    ctx match {
      ...
      case c: ExactlyQuantifierContext =>
        ExactQuantifier(c.n.getText.toInt)
      case c: BetweenQuantifierContext =>
        RangeQuantifier(c.n.getText.toInt, c.m.getText.toInt)
      case c: AtLeastQuantifierContext =>
        MinQuantifier(c.n.getText.toInt)
      case c: AtMostQuantifierContext =>
        RangeQuantifier(0, c.m.getText.toInt)
    }
  }
