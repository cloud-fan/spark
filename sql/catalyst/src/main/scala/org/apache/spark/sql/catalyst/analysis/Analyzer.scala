  ...
  object ResolveTables extends Rule[LogicalPlan] {
    ...
    private def tryResolveV2Table(u: UnresolvedRelation): Option[DataSourceV2Relation] = {
      u.multipartIdentifier match {
        ...
        case CatalogObjectIdentifier(Some(catalogPlugin), ident) =>
          loadTable(catalogPlugin, ident).map(DataSourceV2Relation.create)
    ...
    def apply(plan: LogicalPlan): LogicalPlan = plan.resolveOperatorsUp {
      case i @ InsertIntoStatement(u: UnresolvedRelation, _, _, _, _) if i.query.resolved =>
        tryResolveV2Table(u).map(v2Relation => i.copy(table = v2Relation)).getOrElse(i)

      case u: UnresolvedRelation =>
        tryResolveV2Table(u).getOrElse(u)
    }
  }
