 ...
 * - With bounded repetition: A{3} B{2,5} C{1,}
 * - With alternation: (A | B) C
 * - With grouping: (A B)+ C
 * - With permutation: PERMUTE(A, B) matches A B or B A
 */
...
case class ExactQuantifier(n: Int) extends PatternQuantifier {
...
case class RangeQuantifier(min: Int, max: Int) extends PatternQuantifier {
...
case class MinQuantifier(min: Int) extends PatternQuantifier {
  require(min >= 0, "Minimum repetition must be non-negative")
  override def minOccurrences: Int = min
  override def maxOccurrences: Option[Int] = None
...
/**
 * A permutation pattern (e.g., PERMUTE(A, B, C)).
 * Matches any ordering of the given sub-patterns.
 *
 * Desugared into [[PatternAlternation]] of all permutations during
 * state machine compilation. For example, PERMUTE(A, B) becomes (A B | B A).
 *
 * @param patterns The sub-patterns to permute (must have at least 2)
 */
case class PatternPermute(patterns: Seq[RowPattern]) extends RowPattern {
  require(patterns.size >= 2, "PERMUTE requires at least 2 patterns")

  override def variableNames: Set[String] = patterns.flatMap(_.variableNames).toSet
  override def canBeEmpty: Boolean = patterns.forall(_.canBeEmpty)
  override def patternString: String =
    patterns.map(_.patternString).mkString("PERMUTE(", ", ", ")")
}
