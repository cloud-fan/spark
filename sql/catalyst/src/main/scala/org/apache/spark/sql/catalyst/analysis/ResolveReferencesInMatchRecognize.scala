...
import org.apache.spark.sql.catalyst.plans.logical.{LocalRelation, LogicalPlan, MatchRecognizeVirtualColumns, UnresolvedMatchRecognize}
...
class ResolveReferencesInMatchRecognize(val catalogManager: CatalogManager)
  ...
  def apply(m: UnresolvedMatchRecognize): UnresolvedMatchRecognize = {
    ...
    // Build virtualColumns with unique ExprIds for each qualifier.
    // Only build once (when virtualColumns is empty) to avoid infinite loop.
    ...
    val virtualColumns = if (m.virtualColumns.isEmpty) {
      val attrs = partitionAttrs ++ buildVirtualColumnAttrs(childAttrs, patternVarNames)
      MatchRecognizeVirtualColumns(attrs, partitionAttrs.length, childAttrs.length)
    } else {
      m.virtualColumns
    }
    ...
    // 2. Unqualified virtual column references
    // 3. One for each pattern variable qualifier
    val unqualifiedAttrs = virtualColumns.unqualifiedAttrs
    val qualifiedAttrsByVar = virtualColumns.qualifiedAttrsByVar(patternVarNames.length)

    // Partition columns first (they are directly available like grouping columns),
    // then unqualified virtual columns, then qualified by pattern variable
   ...
   */
  private def buildVirtualColumnAttrs(
