...
private class ScriptTransformationWriterThread(
    ...
    @Nullable inputOi: StructObjectInspector,
    ioschema: HiveScriptIOSchema,
  ...
  override def run(): Unit = Utils.logUncaughtExceptions {
    ...
    try {
      if (inputSerde == null) {
        ...
        val fieldOIs = inputOi.getAllStructFieldRefs.asScala.map(_.getFieldObjectInspector).toArray
        ...
        iter.foreach { row =>
          ...
          val writable = inputSerde.serialize(hiveData, inputOi)
