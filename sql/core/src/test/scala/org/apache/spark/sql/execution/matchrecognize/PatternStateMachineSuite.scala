...
class PatternStateMachineSuite extends SparkFunSuite {
  ...
  test("compile - alternation (A | B)") {
    ...
    // Structure: AltNode(loopPath -> A -> Accept, exitPath -> B -> Accept)
    val alt = sm.startNode.asAlt
    assert(alt.minGroupIterations == 0)

    val a = alt.loopPath.asMatch
    assert(a.variableName == "A")
    assert(a.next == AcceptNode)

    val b = alt.exitPath.asMatch
    assert(b.variableName == "B")
    assert(b.next == AcceptNode)
  }

  test("compile - alternation (A | B | C)") {
    val pattern = PatternAlternation(Seq(
      PatternVariable("A"), PatternVariable("B"), PatternVariable("C")))
    val sm = compilePattern(pattern)

    // Structure: AltNode1(loop -> A, exit -> AltNode2(loop -> B, exit -> C))
    val alt1 = sm.startNode.asAlt
    val a = alt1.loopPath.asMatch
    assert(a.variableName == "A")
    assert(a.next == AcceptNode)

    val alt2 = alt1.exitPath.asAlt
    val b = alt2.loopPath.asMatch
    assert(b.variableName == "B")
    assert(b.next == AcceptNode)

    val c = alt2.exitPath.asMatch
    assert(c.variableName == "C")
    assert(c.next == AcceptNode)
  }

  test("compile - PERMUTE(A, B) desugars to alternation of (A B | B A)") {
    val pattern = PatternPermute(Seq(PatternVariable("A"), PatternVariable("B")))
    val sm = compilePattern(pattern)

    // Structure: AltNode(loop -> A -> B -> Accept, exit -> B -> A -> Accept)
    val alt = sm.startNode.asAlt

    val a1 = alt.loopPath.asMatch
    assert(a1.variableName == "A")
    val b1 = a1.next.asMatch
    assert(b1.variableName == "B")
    assert(b1.next == AcceptNode)

    val b2 = alt.exitPath.asMatch
    assert(b2.variableName == "B")
    val a2 = b2.next.asMatch
    assert(a2.variableName == "A")
    assert(a2.next == AcceptNode)
  }

  test("compile - PERMUTE(A, B, C) desugars to 6 alternatives") {
    val pattern = PatternPermute(Seq(
      PatternVariable("A"), PatternVariable("B"), PatternVariable("C")))
    val sm = compilePattern(pattern)

    // Collect all alternatives by following the AltNode chain
    def collectAlternatives(node: NFANode): Seq[Seq[String]] = node match {
      case alt: AltNode =>
        collectSequence(alt.loopPath) +: collectAlternatives(alt.exitPath)
      case _ =>
        Seq(collectSequence(node))
    }

    def collectSequence(node: NFANode): Seq[String] = node match {
      case m: MatchNode => m.variableName +: collectSequence(m.next)
      case AcceptNode => Seq.empty
      case _ => Seq.empty
    }

    val alternatives = collectAlternatives(sm.startNode)
    assert(alternatives.size == 6)
    assert(alternatives.toSet == Set(
      Seq("A", "B", "C"), Seq("A", "C", "B"),
      Seq("B", "A", "C"), Seq("B", "C", "A"),
      Seq("C", "A", "B"), Seq("C", "B", "A")))
  }
  ...
  test("transitions - non-matching row with min not satisfied is dead end") {
    val pattern = QuantifiedPattern(PatternVariable("A"), MinQuantifier(2), greedy = true)
    val sm = compilePatternWithPredicates(pattern, Map("A" -> 1))
