...
class MatchingEngineSuite extends SparkFunSuite {
  ...
  test("A{3} pattern - exactly 3 matches") {
    val pattern = QuantifiedPattern(PatternVariable("A"), ExactQuantifier(3))
    val engine = new MatchingEngine(
      compilePattern(pattern, Map("A" -> EqualTo(valueAttr, Literal(1)))))

    engine.processRow(createRow(1))
    engine.processRow(createRow(1))
    val result = engine.processRow(createRow(1))
    assert(result.isDefined)
    assert(result.get.getMatchHistory == Seq(M("A", 3)))
  }

  test("A{3} pattern - fails with only 2 matches") {
    val pattern = QuantifiedPattern(PatternVariable("A"), ExactQuantifier(3))
    val engine = new MatchingEngine(
      compilePattern(pattern, Map("A" -> EqualTo(valueAttr, Literal(1)))))
  ...
  test("A{2,4} B pattern - matches minimum (2) then transitions") {
    val pattern = PatternSequence(Seq(
      QuantifiedPattern(PatternVariable("A"), RangeQuantifier(2, 4)),
      PatternVariable("B")))
    val engine = new MatchingEngine(
      compilePattern(pattern, Map(
        "A" -> EqualTo(valueAttr, Literal(1)),
  ...
  test("A{2,4} pattern - greedy takes maximum available") {
    val pattern = QuantifiedPattern(PatternVariable("A"), RangeQuantifier(2, 4))
    val engine = new MatchingEngine(
      compilePattern(pattern, Map("A" -> EqualTo(valueAttr, Literal(1)))))
    ...
    val result = engine.endOfInput()
    assert(result.isDefined)
    assert(result.get.getMatchHistory == Seq(M("A", 4)))
  }

  test("A{2,4} pattern - fails with only 1 match") {
    val pattern = QuantifiedPattern(PatternVariable("A"), RangeQuantifier(2, 4))
    val engine = new MatchingEngine(
      compilePattern(pattern, Map("A" -> EqualTo(valueAttr, Literal(1)))))
  ...
  test("A{2,} pattern - matches at minimum (2) with endOfInput") {
    val pattern = QuantifiedPattern(PatternVariable("A"), MinQuantifier(2))
    val engine = new MatchingEngine(
      compilePattern(pattern, Map("A" -> EqualTo(valueAttr, Literal(1)))))
    ...
    val result = engine.endOfInput()
    assert(result.isDefined)
    assert(result.get.getMatchHistory == Seq(M("A", 2)))
  }

  test("A{2,} pattern - greedily takes all available") {
    val pattern = QuantifiedPattern(PatternVariable("A"), MinQuantifier(2))
    val engine = new MatchingEngine(
      compilePattern(pattern, Map("A" -> EqualTo(valueAttr, Literal(1)))))
    ...
    val result = engine.endOfInput()
    assert(result.isDefined)
    assert(result.get.getMatchHistory == Seq(M("A", 5)))
  }

  test("A{2,} pattern - fails with only 1 match") {
    val pattern = QuantifiedPattern(PatternVariable("A"), MinQuantifier(2))
    val engine = new MatchingEngine(
      compilePattern(pattern, Map("A" -> EqualTo(valueAttr, Literal(1)))))
  ...
  test("A{2,4} B pattern - stops at max and transitions to B") {
    val pattern = PatternSequence(Seq(
      QuantifiedPattern(PatternVariable("A"), RangeQuantifier(2, 4)),
      PatternVariable("B")))
    val engine = new MatchingEngine(
    ...
    val result = engine.processRow(createRow(2))
    assert(result.isDefined)
    assert(result.get.getMatchHistory == Seq(M("A", 4), M("B", 1)))
  }

  test("A{,3} pattern - matches up to max greedily") {
    val pattern = QuantifiedPattern(PatternVariable("A"), RangeQuantifier(0, 3))
    val engine = new MatchingEngine(
      compilePattern(pattern, Map("A" -> EqualTo(valueAttr, Literal(1)))))
  ...
  test("A{,3} B pattern - transitions to B after matching A") {
    val pattern = PatternSequence(Seq(
      QuantifiedPattern(PatternVariable("A"), RangeQuantifier(0, 3)),
      PatternVariable("B")))
    val engine = new MatchingEngine(
      compilePattern(pattern, Map(
        "A" -> EqualTo(valueAttr, Literal(1)),
  ...
  test("A{,3} B pattern - B can match immediately (zero A)") {
    val pattern = PatternSequence(Seq(
      QuantifiedPattern(PatternVariable("A"), RangeQuantifier(0, 3)),
      PatternVariable("B")))
    val engine = new MatchingEngine(
    ...
    val result = engine.processRow(createRow(2))
    assert(result.isDefined)
    assert(result.get.getMatchHistory == Seq(M("B", 1)))
  }

  // ==================== Alternation Pattern Tests ====================

  test("(A | B) pattern - matches first alternative A") {
    val pattern = PatternAlternation(Seq(PatternVariable("A"), PatternVariable("B")))
    val engine = new MatchingEngine(
      compilePattern(pattern, Map(
        "A" -> EqualTo(valueAttr, Literal(1)),
        "B" -> EqualTo(valueAttr, Literal(2)))))
    ...
    val result = engine.processRow(createRow(1))
    assert(result.isDefined)
    assert(result.get.getMatchHistory == Seq(M("A", 1)))
  }

  test("(A | B) pattern - matches second alternative B") {
    val pattern = PatternAlternation(Seq(PatternVariable("A"), PatternVariable("B")))
    val engine = new MatchingEngine(
      compilePattern(pattern, Map(
        "A" -> EqualTo(valueAttr, Literal(1)),
  ...
  test("(A | B) pattern - fails with non-matching row") {
    val pattern = PatternAlternation(Seq(PatternVariable("A"), PatternVariable("B")))
    val engine = new MatchingEngine(
      compilePattern(pattern, Map(
        "A" -> EqualTo(valueAttr, Literal(1)),
  ...
  test("(A | B) C pattern - alternation followed by sequence") {
    val pattern = PatternSequence(Seq(
      PatternAlternation(Seq(PatternVariable("A"), PatternVariable("B"))),
      PatternVariable("C")))
    val engine = new MatchingEngine(
      compilePattern(pattern, Map(
        "A" -> EqualTo(valueAttr, Literal(1)),
    ...
    assert(result.isDefined)
    assert(result.get.getMatchHistory == Seq(M("B", 1), M("C", 1)))
  }

  test("(A | B | C) pattern - matches third alternative") {
    val pattern = PatternAlternation(Seq(
    ...
    val result = engine.processRow(createRow(3))
    assert(result.isDefined)
    assert(result.get.getMatchHistory == Seq(M("C", 1)))
  }
  ...
  test("(A B*)+ pattern - B* matches zero times") {
    val inner = PatternSequence(Seq(
      PatternVariable("A"),
      QuantifiedPattern(PatternVariable("B"), ZeroOrMore, greedy = true)
    ))
    val pattern = QuantifiedPattern(inner, OneOrMore, greedy = true)
    val sm = compilePattern(pattern, Map(
      "A" -> EqualTo(valueAttr, Literal(1)),
      "B" -> EqualTo(valueAttr, Literal(2))
    ))
    val engine = new MatchingEngine(sm)

    engine.processRow(createRow(1))  // matches A, advance to B*
    engine.processRow(createRow(3))  // B* doesn't match, min=0 satisfied

    val result = engine.endOfInput()
    assert(result.isDefined, "(A B*)+ should complete with B* matching 0 times")
    assert(result.get.getMatchHistory == Seq(M("A", 1)))
  }

  test("(A B)* C pattern - zero group iterations") {
    val group = QuantifiedPattern(
      PatternSequence(Seq(PatternVariable("A"), PatternVariable("B"))),
    ...
    val result = engine.processRow(createRow(3))
    assert(result.isDefined, "(A B)* C should match C without any AB iterations")
    assert(result.get.getMatchHistory == Seq(M("C", 1)))
  }
