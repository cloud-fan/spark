...
class MatchRecognizeIterator(
  ...
  private val classifierCache: Map[String, UTF8String] =
    patternVariableDefinitions.map(a => a.name -> UTF8String.fromString(a.name)).toMap
  ...
  private def buildMatchResultsWithGroup(
    ...
    matchHistory.iterator.flatMap { mp =>
      val classifier = classifierCache(mp.variableName)
